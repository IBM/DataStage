...
1. `--prod-apikey-user`: User to login to a custom container registry (or to DataStage Artifactory if --registry is not specified).
1. `--registry`: Custom container registry to pull images from.
