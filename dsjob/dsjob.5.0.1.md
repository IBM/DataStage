...
```
cpdctl dsjob import-zip {--project PROJECT | --project-id PROJID} [--on-failure ONFAILURE] [--conflict-resolution CONFLICT-RESOLUTION] [--skip-on-replace LIST] [--hard-replace] [--asset-type LIST] [--no-compile] [--enable-notification] [--include-binaries] --file-name FILE-NAME [--wait secs] [--enc-key <ENCODING-KEY>]
```
...
```
cpdctl dsjob validate-job {--project PROJECT | --project-id PROJID} {--name NAME | --id ID} [--threads <n>]
```
...
```
cpdctl dsjob send-mail {--recipient RECIPIENT}... --subject SUBJECT --body BODY
```

- `recipient`  list of recipients, at least one recipient should be specified. ex:  --recipient john@ibm.com --recipient peter@ibm.com, 
