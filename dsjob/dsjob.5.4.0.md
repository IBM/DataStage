...
```
cpdctl dsjob update-job {--project PROJECT | --project-id PROJID} {--job NAME | --name NAME | --job-id ID | --id ID} [--to-name RENAME] [--run-name RUNNAME] [--param PARAM] [--param-file FILENAME] [--env ENV] [--paramset NAME] [--jobrun-retention-days DAYS_NUM | --jobrun-retention-runs RUNS_NUM]
```
...
- `jobrun-retention-days` the number of days the job has been running is retained.
- `jobrun-retention-runs` the number of job runs that are retained.
- `to-name` specifies the new name to which the job will be renamed.
...
```
cpdctl dsjob list-scheduled-jobs {--project PROJECT | --project-id PROJID} [--sort | --sort-by-time | --sort-by-scheduled-time] [--with-id] [--all-spaces]
```
...
- `sort-by-scheduled-time` when true, it sorts list of job chronologically in ascending order of next scheduled run.
...
**Note**: Set environment variable ENABLE_CUSTOM_EXITCODE=1 to bypass the standard shell exit code of 1 and instead returns 254 as the exit code, when unexpected errors such a HTTP 502 Bad Gateway error occur.
...
#### Removing Job Invocation ID from Runtime

The following command removes the job invocation ID from ds-runtime.

```
cpdctl dsjob remove-job-invocationid {--project PROJECT | --project-id PROJID}  {{--job NAME | --name NAME} | {--job-id ID | --id ID}} {--invocation-id}
```

- `project` specifies the name of the project that contains the job.
- `project-id` specifies the ID of the project. One of `project` or `project-id` must be provided.
- `name` or `job` specifies the name of the job.
- `id` or `job-id` specifies the ID of the job. One of `job`/`name` or `job-id`/`id` must be provided.
- `invocation-id` specifies the invocation ID of the job.
...
### Getting Full Job Log

The following command retrieves the complete job log for a running or completed job, especially when the log contains a large number of entries.

```
cpdctl dsjob get-joblog {--project PROJECT | --project-id PROJID} {--job NAME | --name NAME | --job-id ID | --id ID} [--run-id RUNID]
```
...
- `name` or `job` specifies the name of the job.
- `id` or `job-id` specifies the ID of the job. One of `job`/`name` or `job-id`/`id` must be provided.
- `run-id` specifies the ID of a specific job run.
...
```
cpdctl dsjob jobrunclean {--project PROJECT | --project-id PROJID} [--name NAME | --id ID] [--run-id RUNID] [--dry-run] [--threads n] [--all-spaces] [--before YYYY-MM-DD:hh:mm:ss] [--states JOBRUNSTATES]
```
...
- `states` should be one of the valid states for cleanup.
- `states` specifies a comma-separated list of active states to process (e.g., `--states Running,Queued,Canceling,Paused,Resuming,Starting`).
- `threads` specifies the number of concurrent tasks to perform; the valid range is 5–20 (default is 5).
...
```
cpdctl dsjob get-jobrun {--project PROJECT | --project-id PROJID} {--name NAME | --id ID} [--run-id RUNID] [--output json|file] [--file-name FILENAME] [--with-metadata] [--compatible] [--run-name] [--skip-metrics]
```
...
- `skip-metrics` when true, it returns job run details skipping metrics. Default is false.
...
```
cpdctl dsjob list-active-runs [[--project PROJECT | --project-id PROJID] | [--space SPACE | --space-id SPACEID]] [--name NAME | --id ID] [--all-spaces] [--sort-by-time|--sort-by-jobname|--sort-by-assettype|--sort-by-duration|--sort-by-state] [--output json|file] [--file-name FILENAME] [--states JOBRUNSTATES]
```
...
- `space` specifies the name of the space; this field is optional.
- `space-id` specifies the ID of the space; this field is optional.
- `states` specifies a comma-separated list of active states to process (e.g., `--states Running,Queued,Canceling,Paused,Resuming,Starting`).
...
```
cpdctl dsjob list-ds-runs {--project PROJECT | --project-id PROJID} {--name name | --id ID}  [--sort-by-jobfolder-name | --sort-by-compiled | --sort-by-assetname] [--show-by-runname] [--output file|json] [--file-name FILENAME --format csv/json] [--detail] [--show-only  [no-runs,not-compiled,out-of-date]]
```
...
- `sort-by-assetname` if set to true, sorts the report by the asset name.
  ...
  **Note:** Only one of `sort-by-jobfolder-name`, `sort-by-compiled`, or `sort-by-assetname` can be specified at a time.
...
- `show-by-runname` when true, shows job runs of the job for each unique job run name. Default is false.

### Generating Job Report with Run Data

This command provides a consolidated report of job runs triggered for each job.

```
cpdctl dsjob get-job-report [--project PROJECT | --project-id PROJID] [--name NAME | --id ID] [--all-spaces] [--output json|file] [--file-name FILENAME] [--sort-by-count]
```

- `project` specifies the name of the project that contains the job; this field is optional.
- `project-id` specifies the ID of the project; this field is optional.
- `name` specifies the name of the job; this field is optional, but when provided, it requires `project` or `project-id`.
- `id` specifies the ID of the job; this field is optional, but when provided, it requires `project` or `project-id`.
- `file-name` specifies the name of the file to which the output is written; this field is optional.

### DSJob API Caching

The DSJob plugin provides a caching mechanism that stores data retrieved from `GET` API calls in local files. On subsequent requests, cached data is used (when available) to avoid unnecessary backend API calls.
...
##### Setup a paraphrase in afile to use for encrypting data in cache files
...
Once Cache is configured, this command can be used to populate Cache for the project.

```
cpdctl dsjob populate-cache {--project PROJECT | --project-id PROJID} [--skip-jobs] [--skip-projects] [--skip-pipelines] [--skip-flows] [--skip-ds-settings] [--skip-container-settings]
```

- `project` specifies the name of the project; this field is optional.
- `project-id` specifies the ID of the project; this field is optional.
- `skip-projects` skips caching projects.
- `skip-jobs` skips caching jobs.
- `skip-flows` skips caching flows.
- `skip-pipelines` skips caching pipelines.
- `skip-ds-settings` skips caching DataStage settings.
- `skip-container-settings` skips caching pipeline settings.

**Use skip flags to avoid caching any changing component.**

Once cached, the cache directory should contain files for that project as shown:
...
For more details, check [DSJob Caching](https://github.com/IBM/DataStage/blob/main/dsjob/blogs/dsjob-cache-mechanism.md).

#### Clearing cached data

Following command clears cached data from the directory.

```
cpdctl dsjob clear-cache {--project PROJECT | --project-id PROJID} [--skip-jobs] [--skip-projects] [--skip-pipelines] [--skip-flows] [--skip-ds-settings] [--skip-container-settings]
```
...
**Use skip flags to avoid clearing cached data for that component component.**
...
- `skip-connections` if true and `conflict-resolution` = replace, use existing connnections with the flows that reference them.
- `migrate-odbc` when set to true, ODBC connectors are converted to native connectors (e.g., `--migrate-odbc sqlServer`).
- `migrate-localfs` when set to true, FileConnector instances with local connections are migrated to a storage volume (e.g., `--migrate-localfs VOLUMENAME`).
...
```
cpdctl dsjob compile-pipeline {--project PROJECT | --project-id PROJID} {--name PIPELINE | --id PIPELINEID} [--code] [--threads <n>] [--enable-inline] [--enable-cache] [--recursive] [--enable-versioning] [--job-suffix] [--unified] [--enable-debug]
```
...
- `enable-debug` when true, it logs extra debug information in the run logs. Default value is false.
...
### Converting Optimized Runner to Unified Pipeline

The following command converts an optimized DataStage pipeline runner job into a unified pipeline job within a project.

```
cpdctl dsjob unify-pipeline {--project PROJECT | --project-id PROJID} {--name NAME | --id ID} [--job-name JOBNAME]
```

- `project` specifies the name of the project.
- `project-id` specifies the ID of the project. One of `project` or `project-id` must be provided.
- `name` specifies the name of the pipeline.
- `id` specifies the ID of the pipeline. One of `name` or `id` must be provided.
- `job-name` specifies the name of the job.

### Managing Unified Pipeline Settings

#### Get Unified Pipeline Settings

This command retrieves Unified Pipeline settings for a project.

```
cpdctl dsjob get-pipeline-settings {--project PROJECT | --project-id PROJID} [--output json|file|table] [--file-name FILENAME]
```
...
#### Set Unified Pipeline Settings

This command allows users to modify existing pipeline settings.

```
cpdctl dsjob update-pipeline-settings {--project PROJECT | --project-id PROJID} [--optimize OPTIMIZE] [--enable-inline ENABLE_INLINE] [--job-suffix JOB_SUFFIX]
```

- `project` specifies the name of the project.
- `project-id` specifies the ID of the project. One of `project` or `project-id` must be provided.
- `optimize` enables or disables the optimized runner in pipeline settings (default: false). Use `--optimize` to enable and `--optimize=false` to disable.
- `enable-inline` determines whether nested pipelines are compiled to run inline. If set to false, they are compiled as independent sequence jobs (default: true).
- `job-suffix` specifies the default pipeline job suffix (e.g., `.Pipeline job`).
...
```
cpdctl dsjob export-project {--project PROJECT | --project-id PROJID} --file-name ZIPFILE  [--no-secrets] [--include-data-assets] [--exclude-datasets-filesets] [--include-binaries] [--enc-key ENCODING-KEY] [--include-compile-metadata] [--wait secs] [--include-python-binaries] [--include-common-binaries]
```
...
- `no-secrets` when set to true, secrets are removed.
- `include-common-binaries` includes compiled optimized pipeline common libraries; the default value is true.
- `include-python-binaries` when set to true, compiled optimized pipeline Python binaries are included.
...
```
cpdctl dsjob import-zip {--project PROJECT | --project-id PROJID} [--on-failure ONFAILURE] [--conflict-resolution CONFLICT-RESOLUTION] [--skip-on-replace LIST] [--hard-replace] [--asset-type LIST] [--no-compile] [--include-compile-errors] [--enable-notification] [--include-binaries] [--run-job-by-name] --file-name FILE-NAME [--wait secs] [--enc-key ENCODING-KEY] [--override-datasets-filesets] [--output FORMAT] [--include-python-binaries] [--include-common-binaries] [--update-job-details]
```
...
- `include-compile-errors` when set to true, the status code is set to 2 to indicate compilation errors.
- `include-common-binaries` includes compiled optimized pipeline common libraries; the default value is true.
- `include-python-binaries` when set to true, compiled optimized pipeline Python binaries are included.
- `update-job-details` when set to true, job details are fully updated, including parameters, environment variables, retention policy, and more.
...
```
cpdctl dsjob export-zip {--project PROJECT | --project-id PROJID} {--name FLOW | --id ID | --pipeline SEQFLOWNAME | --pipeline-id SEQFLOWID | --testcase TESTCASENAME | --testcase-id TESTCASEID | --subflow SUBFLOWNAME | --subflow-id SUBFLOWID} --file-name FILENAME [--no-secrets] [--no-deps] [--include-data-assets] [--exclude-datasets-filesets] [--include-binaries] [--enc-key ENCODING-KEY] [--include-compile-metadata] [--include-python-binaries] [--include-common-binaries]
```
...
- `include-compile-metadata` when true, it includes pipeline compile metadata. Default is false.
- `subflow` specifies the name of the subflow.
- `subflow-id` specifies the ID of the subflow.
- `no-secrets` when set to true, secrets are removed.
- `include-common-binaries` includes compiled optimized pipeline common libraries; the default value is true.
- `include-python-binaries` when set to true, compiled optimized pipeline Python binaries are included.
...
```
cpdctl dsjob export-datastage-assets {--project PROJECT | --project-id PROJID}  --file-name FILENAME [--no-secrets] [--include-data-assets] [--exclude-datasets-filesets] [--include-binaries] [--enc-key ENCODING-KEY] [--include-compile-metadata] [--include-python-binaries] [--include-common-binaries]
```

- `project` is the name of the project.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified.
- `file-name` specifies the name of the .zip file to which the assets are exported.
- `include-data-assets` includes the project data assets as part of the export.
- `include-binaries` when true, runtime and compile time artifacts are included in the exported zip file.
- `exclude-datasets-filesets` when specified datasets and filesets are exported but the data is excluded.
- `enc-key` specifies encryption key used to encrypt exported sensitive data. This key must be a string that will be used during import process to decrypt and must be saved securely.
- `include-compile-metadata` when true, it includes pipeline compile metadata. Default is false.
...
```
cpdctl dsjob view-dataset {--project PROJECT | --project-id PROJID} {--name NAME | --id ID} [--location DATASETLOCATION] [--output json|file --file-name FILENAME --delim CHAR --dump --threads <N> --offset <NUM> --limit <NUM> --prefetch <NUM>]
```
...
- `prefetch` specifies the number of records to cache when the `dump` option is used to optimize fetch operations.
- `thread` specifies the number of concurrent reads to perform; it is only used with the `dump` option (default is 1).
...
```
cpdctl dsjob list-datasets {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} [--sort | --sort-by-time] [--with-id] [--with-location]
```
...
- When `with-location` is specified, the location of the dataset/fileset referenced in `resourceKey` is displayed.
...
```
cpdctl dsjob truncate-dataset {--project PROJECT | --project-id PROJID} {--name NAME | --id ID}  [--location DATASETLOCATION]
```
...
```
cpdctl dsjob rename-dataset {--project PROJECT | --project-id PROJID} {--name NAME | --id ID} [--location DATASETLOCATION] --to-name NEWNAME [--override] [--deep]
```
...
```
cpdctl dsjob list-filesets {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} [--sort | --sort-by-time] [--with-id] [--with-location]
```
...
```
cpdctl dsjob rename-fileset {--project PROJECT | --project-id PROJID} {--name NAME | --id ID} --to-name NEWNAME --deep
```
...
- `deep` if set to true, it renames also the underlying fileset files on disk, default value is false.
...
The following syntax lists remote engines registered in a given DataStage instance.
...
The following syntax unregisters a remote engines from a given DataStage instance.
...
- `id` is the id of the remote engine. One of `name` or `id` must be specified.
...
- `project-id` is the id of the project. One of `project` or `project-id` must be specified. A list of all the build stages in the project is displayed, one per line.
- `sort` when specified returns the list of build stages sorted in alphabetical order. This field is optional.
- `sort-by-time` when specified the list of build stages will be sorted by time of creation with latest at the top of the list. One of `sort` or `sort-by-time` can be specified.
- `with-id` when specified prints the build stage id along with the name of the build stage.
...
```
cpdctl dsjob create-build-stage {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} --name NAME [--description DESCRIPTION] [--folder FOLDERPATH] --file-name ENTITYFILE
```
...
- `file-name` is the name of the file that contains the build stage JSON. The JSON contains the name, description and all other stage details. This field must be specified.
- `folder` is name of the folder where the asset is created, this field is optional
...
- `id` is the id of the build stage. One of `name` or `id` must be specified.
- `output` specifies the format of the output. This field is optional.
- `file-name` specifies the name of the file to which the output is written. This field is optional.
- `with-metadata` when specified adds metadata to the output.
...
- `id` is the id of the build stage. One of `name` or `id` must be specified. Multiple values can be specified for `name` and `id` to delete multiple items, in the format `--name NAME1 --name NAME2...`.
...
The following command allows user to update existing build stage with new definition

```
cpdctl dsjob update-build-stage {--project PROJECT | --project-id PROJID} {--name NAME | --id ID} --file-name FILENAME [--value KEY=VALUE]...
```
...
- `name` is the name of the build stage.  
- `id` is the id of the build stage. One of `name` or `id` must be specified.
- `file-name` name of the file which contains the content used for updating.
- `value` is a name value pair of the individual entity properties that can be updated. This field is repeatable.
...
- `project` is the name of the project that contains the custom stages to list.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified. A list of all the custom stages in the project is displayed, one per line.
- `sort` when specified returns the list of custom stages sorted in alphabetical order. This field is optional.
- `sort-by-time` when specified the list of custom stages will be sorted by time of creation with latest at the top of the list. One of `sort` or `sort-by-time` can be specified.
- `with-id` when specified prints the custom stage id along with the name of the custom stage.
...
```
cpdctl dsjob create-custom-stage {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} --name NAME [--description DESCRIPTION] [--folder FOLDERPATH] --file-name ENTITYFILE
```
...
- `file-name` is the name of the file that contains the custom stage JSON. The JSON contains the name, description and all other stage details. This field must be specified.
- `folder` is name of the folder where the asset is created, this field is optional
...
- `id` is the id of the custom stage. One of `name` or `id` must be specified.
- `output` specifies the format of the output. This field is optional.
- `file-name` specifies the name of the file to which the output is written. This field is optional.
- `with-metadata`when specified adds metadata to the output.
...
- `id` is the id of the custom stage. One of `name` or `id` must be specified. Multiple values can be specified for `name` and `id` to delete multiple items, in the format `--name NAME1 --name NAME2...`.
...
- `project` is the name of the project that contains the wrapped stages to list.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified. A list of all the wrapped stages in the project is displayed, one per line.
- `sort` when specified returns the list of wrapped stages sorted in alphabetical order. This field is optional.
- `sort-by-time` when specified the list of wrapped stages will be sorted by time of creation with latest at the top of the list. One of `sort` or `sort-by-time` can be specified.
- `with-id` when specified prints the wrapped stage id along with the name of the wrapped stage.
...
```
cpdctl dsjob create-wrapped-stage {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} --name NAME [--description DESCRIPTION] [--folder FOLDERPATH] --file-name ENTITYFILE
```
...
- `name` is the name of the custom stage being created.
- `description` is the description of the custom stage being created.
- `file-name` is the name of the file that contains the wrapped stage JSON. The JSON contains the name, description and all other stage details. This field must be specified.
- `folder` is name of the folder where the asset is created, this field is optional
...
- `id` is the id of the wrapped stage. One of `name` or `id` must be specified.
- `output` specifies the format of the output. This field is optional.
- `file-name` specifies the name of the file to which the output is written. This field is optional.
- `with-metadata` when specified adds metadata to the output.
...
- `id` is the id of the wrapped stage. One of `name` or `id` must be specified. Multiple values can be specified for `name` and `id` to delete multiple items, in the format `--name NAME1 --name NAME2...`.
...
- `project-id` is the id of the project. One of `project` or `project-id` must be specified. A list of all the assets in the project is displayed, one per line.
- `asset-type` is the type of asset to list. If not specified, all assets are listed.
- `sort` when specified returns the list of assets sorted in alphabetical order. This field is optional.
- `sort-by-time` when specified the list of assets will be sorted by time of creation with latest at the top of the list. One of `sort` or `sort-by-time` can be specified.
- `with-id` when specified prints the asset id along with the name of the asset.

A status code is printed to the output. A status code of 0 indicates successful completion of the command.

### Getting Asset Info

This command allows users to retrieve detailed information about an asset by name or ID.

```
cpdctl dsjob get-asset-info {--project PROJECT | --project-id PROJID} {--name NAME | --id ID} [--output json|file] [--file-name FILENAME] [--with-metadata] [--type TYPE-EXPRESSION]
```

- `project` specifies the name of the project that contains the assets to list.
- `project-id` specifies the ID of the project. One of `project` or `project-id` must be provided.
- `name` specifies the name of the asset.
- `id` specifies the ID of the asset.
- `type` specifies the type of asset to retrieve using an expression (e.g., `--type "(data_intg_flow OR connection)"`).
- `file-name` specifies the name of the file to which the output is written; this field is optional.
- When `with-metadata` is specified, metadata is included in the output.
...
- `id` is the id of the attachment. One of `name` or `id` must be specified.
- `file-name` specifies the name of the file to which the output is written. This field is optional.
- `with-metadata` when specified adds metadata to the output.
...
- `project` is the name of the project that contains the match specifications to list.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified. A list of all the match specifications in the project is displayed, one per line.
- `sort` when specified returns the list of match specifications sorted in alphabetical order. This field is optional.
- `sort-by-time` when specified the list of match specifications will be sorted by time of creation with latest at the top of the list. One of `sort` or `sort-by-time` can be specified.
- `with-id` when specified prints the match specification id along with the name of the match specification.
...
- `id` is the id of the match specification. One of `name` or `id` must be specified.
- `output` specifies the format of the output. This field is optional.
- `file-name` specifies the name of the file to which the output is written. This field is optional.
- `with-metadata` when specified adds metadata to the output.
...
- `id` is the id of the match specification. One of `name` or `id` must be specified. Multiple values can be specified for `name` and `id` to delete multiple items, in the format `--name NAME1 --name NAME2...`.
...
```
cpdctl dsjob create-function-lib {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} --name NAME [--folder FOLDERPATH] --file-name FILENAME [--return-type-file FILENAME] [--return-type METHODNAME=TYPE]... [--dep-libs FILENAME]...
```
...
```
cpdctl dsjob create-java-library {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} --name NAME --file-name FILENAME [--dep-jar <FILENAME>]... [--folder FOLDERPATH]
```
...
```
cpdctl dsjob create-message-handler {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} --name NAME [--description DESCRIPTION] [--default] [--rule NAME:ACTION:DESCRIPTION] [--folder FOLDERPATH] [--file-name FILENAME]
```
...
The following command allows user to update existing message handler with new definitions.

```
cpdctl dsjob update-message-handler {--project PROJECT | --project-id PROJID} {--name NAME | --id ID} [--description DESCRIPTION] [--default] [--rule NAME:ACTION:DESCRIPTION]... [--file-name FILENAME]
```
...
- `name` is the name of the message handler.
- `id` is the id of the message handler. One of `name` or `id` must be specified.
- `description` updates the description for the message handler. This field is optional.
- `default` when set to true, updates the message handler to be default message handler for the project.
- `rule` defines rules of the message handler. Each rule must have a name, action and description. ex: `IIS-DSEEE-CEID-00001:{promote|promotefatal|demote|suppress}:<DESCR>`.
- `file-name` is the name of the file that contains message handler definition.
...
```
cpdctl dsjob update-ds-settings {--project PROJECT | --project-id PROJID} [--priority Low|Medium|High] [{--env ENVNAME | --env-id ENVID}] [--msg-handler MESSAGEHANDLER] [--separator , or .] [--keep-runs n] [--keep-days n] [--warn-limit] [--time-format hh:nn:ss] [-date-format YYYY-MM-DD] [--timestamp-format YYYY-MM-DD hh:mm:ss] [--nls-format NLSFORMAT] [--nls-collate LANGUAGE] [--enable-remote-engine] [--datastage-job-suffix] [-pipeline-job-suffix] [--pipeline-optimized-suffix]
```
...
- `enable-remote-engine` when set to true, project will switch to remote-engine only mode, and jobs can be run on remote engine environments only. Once true the project can only be used on remote engines and cannot be turned off.
- `datastage-job-suffix` suffix to use when creating DataStage jobs.
- `pipeline-job-suffix` suffix to use when creating Pipeline jobs.
- `pipeline-optimized-suffix` suffix to use when creating Optimized Pipeline runner jobs.
- `warn-limit` specifies the number of warnings and after that a job is terminated. A value of 0 means no limit.
...
- `project` is the name of the project that contains the standardization rules to list.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified. A list of all the standardization rules in the project is displayed, one per line.
...
```
cpdctl dsjob create-rule {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} --name NAME [--description DESCRIPTION] [--location LOCATION] [--folder FOLDERPATH]
```

- `project` is the name of the project that the standardization rule is created for.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified.
- `name` is the name of the standardization rule.
- `description` is the description of the rule.
- `location` is the location folder where the rule is created. Rules can only be created in the folder Customized Standardization Rules.
...
```
cpdctl dsjob export-rule {--project PROJECT | --project-id PROJID} --name NAME --file-name ZIPFILE [--location LOCATION]
```
...
- `name` is the name of the standardization rule.
- `location` is the location of the rule.
- `dest` is the new name of the standardization rule. This is optional and if not specified a default name will be provided.
- `dest-location` is the location of the copied rule. If not specified the location defaults to Customized Standardization Rules.
...
The following syntax lists user volumes in a cluster:
...
The following syntax creates a directory on a volume:
...
The following syntax deletes a directory on a volume:
...
The following syntax lists files on a volume:
...
The following syntax uploads files to a volume:
...
The following syntax downloads files from a volume:
...
```
cpdctl dsjob list-dependencies {--project PROJECT | --project-id PROJID} [--file-name FILENAME] [--deep] [--usedby] [--sort | --sort-by-time] [--output json|file]
```
...
```
cpdctl dsjob list-usage {--project PROJECT | --project-id PROJID} {--name ASSETNAME | --id ASSETID}... [--deep] [--usedby] [--output json|file]
```
...
The following syntax validates pipelines in a specified project to verify that the pipeline references DataStage correctly:

```
cpdctl dsjob validate-pipeline {--project PROJECT | --project-id PROJID} {--name name | --id ID} [--detail] [--threads <n>]
```

- `project` is the name of the project that contains the pipeline.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified.
- `name` is the name of the pipeline.
- `id` is the id of the pipeline. One of `name` or `id` can be specified. If neither is specified, all pipelines in the project are validated.
- `detail` when specified produces a detailed output. This field is optional.
- `threads` specifies the number of concurrent validations to run, this value can be between 5-20. Default values is 5.
...
```
cpdctl dsjob validate-connection {--project PROJECT | --project-id PROJID} {--name NAME | --id ID} [--threads <n>]
```

- `project` is the name of the project.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified.
- `name` is the name of the connection.
- `id` is the id of the connection. If both `name` and `id` are not specified, all connections in the project are validated.
- `threads` specifies the number of concurrent validations to run, this value can be between 5-20. Default values is 5.
...
```
cpdctl dsjob validate-flow {--project PROJECT | --project-id PROJID} {--name NAME | --id ID} [--threads <n>]
```
...
```
cpdctl dsjob validate-subflow {--project PROJECT | --project-id PROJID} {--name NAME | --id ID} [--threads <n>]
```
...
```
cpdctl dsjob validate-job {--project PROJECT | --project-id PROJID} {--name NAME | --id ID} [--threads <n>] [--remove-param-overrides]
```
...
- `threads` specifies the number of concurrent validations to run, this value can be between 5-20. Default values is 5.
- `remove-param-overrides` when true, will replace the job parameters with flow runtime parameters if they are different.
...
```
cpdctl dsjob create-cff-schema {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} --name NAME [--description Description] [--folder FOLDERPATH] --file-name SCHEMAFILENAME
```
...
```
cpdctl dsjob list-folders {--project PROJECT | --project-id PROJID} [--sort | --sort-by-time] [--with-id] [--output json]
```
...
- `output` when specified, the `json` value is accepted as a valid value. When used, the command output results in `json`.
...
The following command lists all data assets in a project. A list of all the data assets in the project are displayed, one per line.

```
cpdctl dsjob list-data-assets {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} [--sort | --sort-by-time | --sort-by-type] [--with-id]
```

- `project` is the name of the project that contains the data assets to list.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified.
- `sort` when specified returns the list of data assets sorted in alphabetical order. This field is optional.
- `sort-by-time` when specified, the list of data assets will be sorted by time of creation with latest at the top of the list.
- `sort-by-type` when specified, the list of data assets will be sorted by type of data asset. One of `sort`, `sort-by-time` or `sort-by-type` can be specified.
- `with-id` when specified prints the data asset id along with the name of the data asset.

A status code is printed to the output. A status code of 0 indicates successful completion of the command.

### Deleting Data Assets

The following command deletes a data assets in a project.

```
cpdctl dsjob delete-data-asset {--project PROJECT | --project-id PROJID} {--name NAME | --id ID}...
```

- `project` is the name of the project that contains the data assets to list.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified.
- `name` is the name of the data asset that is being deleted.
- `id` is the id of the data asset. One of `name` or `id` must be specified. Multiple values can be specified for `name` and `id` to delete multiple items, in the format `--name NAME1 --name NAME2...`.

A status code is printed to the output. A status code of 0 indicates successful completion of the command.

### Upload data asset

The following syntax uploads a data asset to a project. The data asset along with schema and data is created on the target project.

```
cpdctl dsjob upload-data-asset {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} --name NAME [--description DESCRIPTION] --mime MIMETYPE  [--tag TAG]... --file-name file
```

- `project` is the name of the project.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified.
- `name` name of the data asset to create
- `description` description of the data asset to create
- `file-name` name of the .zip file with data asset content.
- `mime` is the type of the data asset to upload ex: text/csv
- `tag` is used to tag the asset for fast search. This field is repeatable and an asset can have multiple tags to categorize.

The asset id of the newly created data asset is printed to the output. A status code is printed to the output. A status code of 0 indicates successful completion of the command.
...
The following syntax exports data assets by name from the specified project. The exported zip file is compatible with `import-zip`. Please refer to the `import-zip` command to import as desired into a different project.

```
cpdctl dsjob export-data-asset {--project PROJECT | --project-id PROJID} --name NAME --file-name ZIPFile
```

- `project` is the name of the project.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified.
- `name` name of the data asset to export. `name` can be a regular expression. Example: `abc.*` exports all names that start with abc.
- `file-name` name of the .zip file to which exported data assets are written.
...
Following command allows user to configure the git integration for a project. A project should be configured to a git repository of choice with credential provided in the for of git token such that subsequent git commands can use this information for connecting and performing operations against the repo.

```
cpdctl dsjob git-configure {--project PROJECT | --project-id PROJID} [--git-enable] [--git-url URL] [--git-owner OWNER] [--git-token TOKEN] [--git-org ORG] [--git-email EMAIL] [--git-provider GITPROVIDER]  [--git-branch GITBRANCH]  [--git-folder GITFOLDER] [--git-cert | --git-certfile] [--git-cert-key | --git-cert-keyfile]
```

- `project` is the name of the project.
- `project-id` is the id of the project. One of  `project`  or  `project-id`  must be specified.
- `git-enable` when true sets the project to be git enabled
- `git-url` https url of the repo
- `git-owner` the user name  of the owner of the repo, this field is required when configuring to bitbucket.
- `git-org` the  name  of the oraganization of the repo, this field is optional
- `git-token` the git token used for invoking git operations against the repo
- `git-email` email of the user for repo notification, this field is optional
- `git-provider` must be one for `git`, `gitlab` or `bitbucket` , although no validation is performed when configuring this field should be correctly configured.
- `git-branch` branch to which project will be uploaded. This field is optional.
- `git-folder` name of a folder in a git repo where the project content are stored. This field is optional.
- `git-cert` the certificate is used when git server is SSL enabled.
- `git-certfile` file name that contains the certificate that is used when git server is SSL enabled. Only one of `git-cert` to `git-certfile` should be used.
- `git-cert-key` the  certificate key that is used with certificate when git server is SSL enabled
- `git-cert-keyfile` file name that contains certificate key used with certificate when git server is SSL enabled. Only one of `git-cert-key` to `git-cert-keyfile` should be used.

A status code is printed to the output. A status code of 0 indicates successful completion of the command.

## Get git configuration

The following command provides the current git configuration of a given project.

```
cpdctl dsjob get-git-configure {--project PROJECT | --project-id PROJID}
```
...
```
cpdctl dsjob git-commit {--project PROJECT | --project-id PROJID} [--repo REPONAME] [--branch BRANCH] [--in-folder FOLDERNAME] [--commit-message MESSAGE] [--use-zip ZIPFILE] [--name ASSETNAME | --id ASSETID]... [--include-data-assets] [--exclude-datasets-filesets] [include-binaries] [--enc-key ENCODING-KEY] [--wait]
```
...
- `exclude-datasets-filesets` when specified datasets and filesets are exported but the data is excluded.
- `enc-key` specifies encryption key used to encrypt exported sensitive data. This key must be a string that will be used during import process to decrypt and must be saved securely.
- `include-binaries` export runtime and compile artifacts when set. Default value is true.
- `wait` specify the time in seconds to wait for the command to complete. For example, `--wait 200` waits for a maximum of 200 secs, checking status of the `git-commit`, and returns an exit code other than zero if it does not complete. You can set `--wait -1` to wait for completion indefinitely.

**Note**: Users cannot commit an empty project.

### Get status of a Git Commit

The following command get status on a git-commit in progress.

```
cpdctl dsjob get-git-commit {--project PROJECT | --project-id PROJID} [--output json|file] [--file-name FILENAME]
```
...
```
cpdctl dsjob git-pull {--project PROJECT | --project-id PROJID} [--repo REPONAME] [--in-folder FOLDERNAME] [--branch BRANCH] [--name ASSETNAME | --id ASSETID]... [--on-failure ONFAILURE] [--conflict-resolution CONFLICT-RESOLUTION] [--skip-on-replace LIST] [--hard-replace] [--no-compile] [--enable-notification] [--include-binaries] [--run-job-by-name] [--enc-key ENCODING-KEY]
```
...
- `hard-replace` If true, fields in parameter sets will be replaced with incoming source field values. This field can be used only when conflict resolution is set to skip or replace.
- `no-compile` if true, flows imported will not be compiled.
- `enable-notification` when true, allows notifications to be sent during git-pull.
- `include-binaries` when true, runtime and compile time artifacts are included in the pull.
- `run-job-by-name` when specified the job is imported with job name instead of job id.
- `enc-key` when specified, encoded data is imported using this key.

**Note**: When cloning from a repository, only partial deletion of project assets is allowed.

### Get status of a Git Pull

The following command get status on a git-pull in progress.

```
cpdctl dsjob get-git-pull {--project PROJECT | --project-id PROJID} --import-id [--output json|file] [--file-name FILENAME]
```
...
```
cpdctl dsjob git-status {--project PROJECT | --project-id PROJID} [--repo REPONAME] [--in-folder FOLDERNAME] [--branch BRANCH] [--hide-identical] [--hide-modified] [--hide-deleted] 
```
...
- `repo` name of the git repo.
- `in-folder` name of a folder in a Git Repo where the project content are stored. This field is optional.
- `hide-identical` when true, output will ignore identical assets from display, default is false.
- `hide-modified` when true, output will ignore modified assets from display, default is false.
- `hide-deleted` when true, output will ignore deleted assets from display, default is false.

### Managing git project tracking data

Git Projects, when configured, start monitoring the project for changes to various objects in the project. This information is kept on a local PVC in case of CP4D environment. This is used to detect what objects have changed between Git and a Project and help properly orchestrate a commit or a pull. Make sure that only projects that user want to run git operation are tracked. This command allows user to monitor and turn off tracking if they are not interested in continual monitoring on some unwanted projects.

#### List Projects that are git enables and monitored using project tracking

Following command returns the list of projects that are currently tracked and monitored for changes.

```
cpdctl dsjob list-track-project
```

This command does not take any arguments and lists all projects that are git enabled and are monitored.

#### Enable or Disable project tracking

The following command can be used the enable to disable project tracking for a specified project.

```
cpdctl dsjob git-track-project {--project PROJECT | --project-id PROJID} --disable
```

- `project` is the name of the project.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified.
- `disable` when set to true, it disables the project from tracking and deletes all tracked data for the project. Default value is true.

### Using Git Tokens from User Profile

#### Get Git Token from User Profile

```
cpdctl dsjob get-git-token --provider PROVIDER
```

- `provider` specifies the Git provider. Valid values are `git`, `gitlab`, `bitbucket`, and `azure`.

#### Set Git Token in User Profile

```
cpdctl dsjob set-git-token --provider PROVIDER --token TOKEN --name TOKENNAME --domain DOMAIN
```

- `provider` specifies the Git provider; one of `git`, `gitlab`, `bitbucket`, or `azure`.
- `token` specifies the Git token for the user.
- `name` specifies the name of the Git token for the user.
- `domain` specifies the Git domain to operate on.
...
## Configuring Project Metrics

### Update-metrics Update Datastage project metrics settings

The following command can be used to update metrics configuration of a project

```
cpdctl dsjob update-metrics {--project PROJECT | --project-id PROJID} {--enable | {--db-password DB-PASSWORD [--db-type DB-TYPE] [--db-host DB-HOST] [--db-port DB-PORT] [--db-name DB-NAME] [--db-user DB-USER] [--query-timeout QUERY-TIMEOUT] [--retry-limit RETRY-LIMIT] [--ssl --ssl-certificate-validation [--ssl-cert SSL-CERT] [--ssl-certificate-host CERTIFICATE-HOST] [--private-key PRIVATE-KEY] [--key-certificate KEY-CERTIFICATE]] }}
```

- `project` is the name of the project.
- `project-id` is the id of the project. One of `project`, `project-id`, `space` or `space-id` must be specified.
- `space` is the name of the space.
- `space-id` is the id of the space.
- `enable` when set to true, enables metrics for the project
- `db-password` password of the user to the metrics database
- `db-type` datasource type for the database
- `db-host` database host name
- `db-port` port number of the database
- `db-name` name of the database
- `db-user` name of the user for database conection
- `query-timeout` key query timeout
- `retry-limit` retry limit
- `ssl` enable SSL connection (must be active to use other SSL options)
- `ssl-cert` SSL Certification
- `ssl-certificate-host` SSL certificate host
- `ssl-certificate-validation` validate the returned by host SSL Cert as trusted (default to false, if true, must be an input)
- `private-key` is the private key of a postgresql connection. This field is optional.
- `key-certificate` is the key certificate of a private key. This field is optional.

### Display Metrics Configuration

The following command display metrics configuration of a project.

```
cpdctl dsjob get-metrics {--project PROJECT | --project-id PROJID}
```
...
The following command disables metrics configuration of a project.

```
cpdctl dsjob disable-metrics {--project PROJECT | --project-id PROJID}
```
...
The following command will enable project indexing once metrics database is configured.

```
cpdctl dsjob enable-project-indexing {--project PROJECT | --project-id PROJID} [--enable ENABLE]
```

- `project`  is the name of the project.
- `project-id`  is the id of the project. One of  `project`  or  `project-id`  must be specified.
- `enable` when set to true, it enables indexing assets in a  project to facilitate fast search capabilities on the project. For enabling the project we must have project metrics configured and this command checks to see if the connection is valid before enabling indexing.

A status code is printed to the output. A status code of 0 indicates successful completion of the command.

### Start Project Indexing

The following command will start running project indexing, if the indexing is already enabled on the project.

```
cpdctl dsjob index-project {--project PROJECT | --project-id PROJID} [--details] [--wait secs]
```

- `project`  is the name of the project.
- `project-id`  is the id of the project. One of  `project`  or  `project-id`  must be specified.
- `details` when true, it provides more detailed status on the state of indexing the assets in a project. This option is valid when command is run with `wait` option.
- `wait` total wait time in seconds to wait after start indexing while waiting to finish indexing. A value of `-1` will wait until indexing is complete while providing status periodically.

A status code is printed to the output. A status code of 0 indicates successful completion of the command.

### Get Status on Project Indexing

The following command will fetch the status of  project indexing.

```
cpdctl dsjob get-index-status {--project PROJECT | --project-id PROJID} [--details]
```

- `project`  is the name of the project.
- `project-id`  is the id of the project. One of  `project`  or  `project-id`  must be specified.
- `details` when set to true, provides more details on the status of the project indexing such as progress on number of assets indexed.

A status code is printed to the output. A status code of 0 indicates successful completion of the command.

### Testing Repository Connections

The following commands tests if the DataStage repository connection is configured correctly. It is recommended to run this command before enabling project indexing.
```
cpdctl dsjob test-repository-connection {--project PROJECT | --project-id PROJID}
```
...
The following command allows sending a mail to a group of recipients.

```
cpdctl dsjob send-mail {--recipient RECIPIENT}... --subject SUBJECT --body BODY [--attachment FILENAME]...
```

- `attachment` List of attachments to be sent, upto 3 attachments allowed. Example: --attachment /path/abc --attachment /path/pqr
- `recipient` list of recipients to who email will be sent, at least one recipient should be specified. Example: --recipient <john@ibm.com> --recipient <peter@ibm.com>,
- `subject` subject of the email to send
- `body` the content of the email body

## Clearing Vault Cache

The following command clears the vault cache before the cache refreshes during timeout.
...
### Getting WLM Configuration

The following syntax gets the current WLM configuration of the specified project.

```
cpdctl dsjob get-wlmconfig {--project PROJECT | --project-id PROJID} --name NAME [--output json|file] [--file-name FILENAME]
```

- `project is` the name of the project that the message handler is created for.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified.
- `name` is the name of the PX Instance, custom resource instance name, ex: ds-px-default
- `output` specifies the format of the output. This field is optional.
- `file-name` specifies the name of the file to which the output is written. This field is optional.

### Updating WLM Params

The following command is used to update WLM Configuration params.

```
cpdctl dsjob update-wlmparams {--project PROJECT | --project-id PROJID} --name PXINSTANCE [--param NAME=VALUE...] [--param-file FILENAME...]
```

- `project` is the name of the project that the message handler is created for.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified.
- `name` name of the PX runtime instance on which the WLM params are updated.
- `param` list of params to update in name=value format. This field is optional.
- `param-file` specifies the name of the file that contains one param per line in name-value format. This field is optional.

### Updating WLM Resources

The following command is used to update WLM Configuration resources.

```
cpdctl dsjob update-wlmresources {--project PROJECT | --project-id PROJID} --name PXINSTANCE [--resource NAME=VALUE...] [--resource-file FILENAME...]
```

- `project` is the name of the project that the message handler is created for.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified.
- `name` name of the PX runtime instance on which the WLM resources are updated.
- `resource` list of resources to update in name=value format. This field is optional.
- `resource-file` specifies the name of the file that contains one resource per line in name-value format. This field is optional.

### Updating WLM Queues

The following command is used to update WLM Configuration queues.

```
cpdctl dsjob update-wlmqueues {--project PROJECT | --project-id PROJID} --name PXINSTANCE [--queue NAME=DEFN-JSON...] [--file-name FILENAME...] [--del-queue QUEUENAME...]
```

- `project is` the name of the project that the message handler is created for.
- `project-id` is the id of the project. One of `project` or `project-id` must be specified.
- `name` name of the PX runtime instance on which the WLM queues are updated.
- `queue` name of the queue and its definition as json in the name-definition format. This field is optional.
- `file-name` specifies the name of the file that contains queue definition as name-value format. This field is optional.
- `del-queue` delete queue with a given name. This field is optional.
