...
Supported using pipeline plugin. Refer to `cpdctl pipeline cleanup` command group
...
```
cpdctl dsjob list-data-assets {--project PROJECT | --project-id PROJID} [--sort | --sort-by-time] [--with-id]
```
...
- `sort-by-time` when specified, the list of data assets will be sorted by time of creation with latest at the top of the list. One of `sort`or `sort-by-time` can be specified.
...
```
cpdctl dsjob upload-data-asset {--project PROJECT | --project-id PROJID} --name NAME [--description DESCRIPTION] --file-name file
```
