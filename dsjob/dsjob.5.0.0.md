...
```
cpdctl dsjob update-metrics {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} {--enable | {--db-password DB-PASSWORD [--db-type DB-TYPE] [--db-host DB-HOST] [--db-port DB-PORT] [--db-name DB-NAME] [--db-user DB-USER] [--query-timeout QUERY-TIMEOUT] [--retry-limit RETRY-LIMIT] [--ssl --ssl-certificate-validation [--ssl-cert SSL-CERT] [--ssl-certificate-host CERTIFICATE-HOST] [--private-key PRIVATE-KEY] [--key-certificate KEY-CERTIFICATE]] }}
```
