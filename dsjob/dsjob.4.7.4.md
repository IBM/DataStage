...
```
 cpdctl dsjob migrate {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} [--on-failure ONFAILURE] [--conflict-resolution CONFLICT-RESOLUTION] [--attachment-type ATTACHMENT-TYPE] [--import-only] [--create-missing] [--enable-local-connection] [--enable-platform-connection] [--enable-dataquality-rule] [--create-connection-paramsets] [--use-dsn-name] [--file-name FILENAME] [--status IMPORT-ID --format csv/json] [--stop IMPORT-ID] [--hard-replace] [--wait secs]
```
...
```
cpdctl dsjob compile {--project PROJECT | --project-id PROJID} [{--name NAME | --id ID}...] [--osh] [--threads ] [--enable-elt-mode --materialization-policy <POLICY>]
```
...
- `rule` defines rules of the message handler. Each rule must have a name, action and description.
...
```
cpdctl dsjob update-ds-settings {--project PROJECT | --project-id PROJID} [--priority <Low, Medium or High>] {--env ENVNAME | --env-id ENVID}... [--msg-handler <MESSAGEHANDLER>] [--separator <, or .>] [--keep-run <N>] [--keep-days <N>] [--time-format <hh:nn:ss>] [-date-format <yyyy-mm-dd>] [--timestamp-format <yyyy:mm:dd hh:nn:ss>] [--nls-format <NLSFORMAT>]
```

- `project`  is the name of the project.
- `project-id`  is the id of the project. One of  `project`  or  `project-id`  must be specified.
- `priority` sets the priority on the DataStage job. Valid values are Low, Medium and High. This field is optional.
-  `env` name of the environment that users can access to run their job. This field is optional.
-  `env-id` id of the environment that users can access to run their job. This field is optional.
...
```
cpdctl dsjob export-rules {--project PROJECT | --project-id PROJID} --name NAME --file-name ZIPFile
...
- `to-file` if specified, uses this value as the name of the destination file.
...
volume:
```
cpdctl dsjob download-volume-files --name [--path] [--file-name ] [--output-file ]
```
- `name` is the name of the volume.
- `path` is a path on the volume. This field is optional. 
...
```
cpdctl dsjob list-dependencies {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} [--file-name FILENAME] [--deep] [--usedby] [--sort | --sort-by-time]
```
...
```
cpdctl dsjob create-dsparams {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} {--name NAME | --id ID}  [--file-name FILENAME]
```
...
```
cpdctl dsjob validate-pipeline {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} {--name name | --id ID} [--detail]
```
...
```
cpdctl dsjob validate-connection {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} {--name NAME | --id ID}
```
...
```
cpdctl dsjob validate-flow {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} {--name NAME | --id ID}
```
...
```
cpdctl dsjob validate-subflow {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} {--name NAME | --id ID}
```
...
```
cpdctl dsjob validate-job {{--project PROJECT | --project-id PROJID} | {--space SPACE | --space-id SPACEID}} {--name NAME | --id ID}
```
